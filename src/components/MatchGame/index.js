...
class MatchGame extends Component {
  ...
  onImageClick = id => {
    ...
    this.setState({
      imgId: imagesList[Math.floor(Math.random() * imagesList.length)].id,
    })
  }

  RenderRandomImage = () => {
    const {imgId} = this.state
    const image = imagesList.filter(eachImage => eachImage.id === imgId)
    return <img src={image[0].imageUrl} alt="match" className="match-img" />
  }
  ...
  render() {
    const {activeTab, score, timer, status} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <div className="bg-container">
          ...
          {status === 'over' && (
            <OverItem score={score} resetGame={this.resetGame} />
          )}
        </div>
      </div>
    )
